...
const BrandList = () => {
  ...
  const getAllBrands = async ({ page = 1 }) => {
    try {
      ...
      let response = await brandSvc.listAllBrand({ page });
      ...
      toast.error("Error fetching Brand List..");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteBrand = async (id) => {
    try {
      ...
      getAllBrands({ page: +pagination.currentPage });
    } catch (exception) {
  ...
  return (
    <>
      <Container fluid className="px-4">
        <Heading className={"mt-4"} value={"Brand List"} type={"h1"}></Heading>

        <BreadCrumb
          data={[
            { title: "Dashboard", url: "/admin" },
            { title: "Brand List", url: null },
          ]}
        ></BreadCrumb>
        <Card className="mb-4">
          <Card.Header>
